...
class SubspaceClusterNetwork(nn.Module):
    def __init__(self,backbone_name,z_dim,n_clusters,norm_p=2,normalize=True):
        ...
        self.cluster = nn.Sequential(nn.Linear(4096,4096), nn.ReLU(), nn.Linear(4096, n_clusters))
#         self.cluster.weight = nn.Parameter(torch.zeros_like(self.cluster.weight))
        #self.subspace = nn.Linear(4096,z_dim)
        
        self.subspace = nn.Sequential(nn.Linear(4096,4096), nn.ReLU(),  nn.Linear(4096, z_dim))
...
class SelfSupervisedNetwork(nn.Module):
    def __init__(self,backbone_name,z_dim,classifier=False,n_classes=10,normalize=True,norm_p=2):
        ...
        self.normalize = normalize
        if classifier:
            self.classifier = nn.Sequential(nn.Linear(4096,4096), nn.ReLU(), nn.Linear(4096,4096), nn.ReLU(), nn.Linear(4096, n_classes))
        else:
            #self.projection = nn.Sequential(nn.Linear(4096,4096), nn.ReLU(), nn.Linear(4096,4096), nn.ReLU(), nn.Linear(4096, z_dim))
            self.projection = nn.Sequential(nn.Linear(4096,4096), nn.ReLU(), nn.Linear(4096, z_dim))
            
            #self.projection = nn.Sequential(nn.Linear(4096,4096),  nn.BatchNorm1d(4096), nn.ReLU(), nn.Linear(4096, z_dim))
    ...
    def forward(self,x,detach_feature=False,return_feature=None):
        ...
        if self.classifier:
            ...
            if self.normalize:
                ...
                return self.projection(self.drop(feature))
